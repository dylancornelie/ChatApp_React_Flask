...
class WsMessageNamespace(Namespace):
    ...
    def on_leave_project(self, data):
        ...
        self.emit('offline', data=dict(user_id=data_leave.get('user_id')), room=data_leave.get('room'), include_self=False)
    ...
    def on_send_message(self, data):
        ...
        if not message.receiver_id:
            ...
            sid_receive = get_sid_by_user_id_in_room(message.receiver_id, data.get('room'))
            if sid_receive:
